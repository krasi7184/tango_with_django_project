...
import os
# We use regular expressions to do more in-depth checks on generated HTML output from views.
import re
import warnings
import importlib
...
class Chapter6PopulationScriptTest(TestCase):
    ...
    def test_page_objects_have_views(self):
        ...
        for page in pages:
            self.assertTrue(
                page.views > 0, f"{FAILURE_HEADER}The page '{page.title}' has a negative/zero view count. The exercises for Chapter 6 stated that all view values must be greater than zero. Update your population script, and try again.{FAILURE_FOOTER}")
...
class Chapter6IndexViewTests(TestCase):
    ...
    def test_index_context_dictionary(self):
        ...
        expected_categories_order = list(
            Category.objects.order_by('-likes')[:5])
        # From the exercises section of Chapter 6 -- we cannot assume a set order, because the developer can set the number of views to whatever they wish.
        ...
        self.assertEqual(expected_categories_order, list(
            self.response.context['categories']), f"{FAILURE_HEADER}Incorrect categories/category order returned from the index() view's context dictionary -- expected {expected_categories_order}; got {list(self.response.context['categories'])}.{FAILURE_FOOTER}")
        ...
        self.assertEqual(expected_pages_order, list(
            self.response.context['pages']), f"{FAILURE_HEADER}The 'pages' context dictionary variable for the index() view didn't return the QuerySet we were expectecting: got {list(self.response.context['pages'])}, expected {expected_pages_order}. Did you apply the correct ordering to the filtered results?{FAILURE_FOOTER}")
    ...
    def test_index_categories(self):
        ...
        for entry in category_li_entries_regex:
            self.assertTrue(re.search(
                entry[0], self.content), f"{FAILURE_HEADER}We couldn't find the expected markup '{entry[2]}' (for the {entry[1]} category) in the response of your index() view. Check your template, and try again.{FAILURE_FOOTER}")
    ...
    def test_index_pages(self):
        ...
        for expected_page in expected_pages_order:
            expected_pages_li.append(
                page_li_entries_regex[expected_page.title])
...
class Chapter6NoItemsIndexViewTests(TestCase):
    ...
    def test_empty_index_context_dictionary(self):
        ...
        self.assertEqual(len(
            self.response.context['pages']), 0, f"{FAILURE_HEADER}The 'pages' variable in the context dictionary for index() is not empty. (Empty check){FAILURE_FOOTER}")
    ...
    def test_sample_category(self):
        ...
        updated_response = self.client.get(
            reverse('rango:index')).content.decode()
...
class Chapter6CategoryViewTests(TestCase):
    ...
    def setUp(self):
        populate()
        self.response = self.client.get(reverse('rango:show_category', kwargs={
                                        'category_name_slug': 'other-frameworks'}))
        self.content = self.response.content.decode()
    ...
    def test_context_dictionary(self):
        ...
        """
        other_frameworks_category = Category.objects.get_or_create(
            name='Other Frameworks')[0]
        page_list = list(Page.objects.filter(
            category=other_frameworks_category))
...
class Chapter6BadCategoryViewTests(TestCase):
    ...
    def test_nonexistent_category(self):
        ...
        """
        response = self.client.get(reverse('rango:show_category', kwargs={
                                   'category_name_slug': 'nonexistent-category'}))
    ...
    def test_empty_category(self):
        ...
        response = self.client.get(reverse('rango:show_category', kwargs={
                                   'category_name_slug': 'test-category'}))

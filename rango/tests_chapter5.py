...
class Chapter5DatabaseConfigurationTests(TestCase):
    ...
    def test_databases_variable_exists(self):
        ...
        """
        self.assertTrue(
            settings.DATABASES, f"{FAILURE_HEADER}Your project's settings module does not have a DATABASES variable, which is required. Check the start of Chapter 5.{FAILURE_FOOTER}")
    ...
    def test_gitignore_for_database(self):
        ...
        if git_base_dir.startswith('fatal'):
            ...
            if os.path.exists(gitignore_path):
                self.assertTrue(self.does_gitignore_include_database(
                    gitignore_path), f"{FAILURE_HEADER}Your .gitignore file does not include 'db.sqlite3' -- you should exclude the database binary file from all commits to your Git repository.{FAILURE_FOOTER}")
            else:
                warnings.warn(
...
class Chapter5ModelTests(TestCase):
    ...
    def setUp(self):
        category_py = Category.objects.get_or_create(
            name='Python', views=123, likes=55)
    ...
    def test_page_model(self):
        ...
        self.assertEqual(
            page.views, 156, f"{FAILURE_HEADER}Tests on the Page model failed. Check you have all required attributes (including those specified in the exercises!), and try again.{FAILURE_FOOTER}")
...
class Chapter5AdminInterfaceTests(TestCase):
    ...
    def setUp(self):
        ...
        User.objects.create_superuser(
            'testAdmin', 'email@email.com', 'adminPassword123')
        ...
        Page.objects.get_or_create(
            title='TestPage1', url='https://www.google.com', category=category)
...
class Chapter5PopulationScriptTests(TestCase):
    ...
    def test_categories(self):
        ...
        self.assertEqual(
            categories_len, 3, f"{FAILURE_HEADER}Expecting 3 categories to be created from the populate_rango module; found {categories_len}.{FAILURE_FOOTER}")
    ...
    def test_pages(self):
        ...
        details = {'Python':
                   ...
                   'Django':
                       ['Official Django Tutorial', 'Django Rocks',
    ...
    def test_counts(self):
        ...
        for category in details:
            ...
            self.assertEqual(
                category.views, values['views'], f"{FAILURE_HEADER}The number of views for the '{category}' category is incorrect (got {category.views}, expected {values['views']}, generated from populate_rango).{FAILURE_FOOTER}")
            self.assertEqual(
                category.likes, values['likes'], f"{FAILURE_HEADER}The number of likes for the '{category}' category is incorrect (got {category.likes}, expected {values['likes']}, generated from populate_rango).{FAILURE_FOOTER}")
    ...
    def check_category_pages(self, category, page_titles):
        ...
        self.assertEqual(pages_len, len(
            page_titles), f"{FAILURE_HEADER}Expected {page_titles_len} pages in the Python category produced by populate_rango; found {pages_len}.{FAILURE_FOOTER}")
